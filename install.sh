...
function system_check() {
  ...
  if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
    ...
    INS="yum install -y"
    ${INS} wget
    wget -N -P /etc/yum.repos.d/ https://raw.githubusercontent.com/schild/Xray_onekey/${github_branch}/basic/nginx.repo
...
function dependency_install() {
  ...
  if ! command -v jq; then
    wget -P /usr/bin https://raw.githubusercontent.com/schild/Xray_onekey/${github_branch}/binary/jq && chmod +x /usr/bin/jq
    judge "安装 jq"
  fi
...
function update_sh() {
  ol_version=$(curl -L -s https://raw.githubusercontent.com/schild/Xray_onekey/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [[ "$shell_version" != "$(echo -e "$shell_version\n$ol_version" | sort -rV | head -1)" ]]; then
    print_ok "存在新版本，是否更新 [Y/N]?"
    read -r update_confirm
    case $update_confirm in
    [yY][eE][sS] | [yY])
      wget -N --no-check-certificate https://raw.githubusercontent.com/schild/Xray_onekey/${github_branch}/install.sh
      print_ok "更新完成"
...
function configure_nginx() {
  ...
  cd /etc/nginx/conf.d/ && rm -f ${domain}.conf && wget -O ${domain}.conf https://raw.githubusercontent.com/schild/Xray_onekey/${github_branch}/config/web.conf
...
function configure_xray() {
  cd /usr/local/etc/xray && rm -f config.json && wget -O config.json https://raw.githubusercontent.com/schild/Xray_onekey/${github_branch}/config/xray_xtls-rprx-vision.json
  modify_UUID
  modify_port
}

function configure_xray_ws() {
  cd /usr/local/etc/xray && rm -f config.json && wget -O config.json https://raw.githubusercontent.com/schild/Xray_onekey/${github_branch}/config/xray_tls_ws_mix-rprx-vision.json
  modify_UUID
  modify_UUID_ws
  modify_port
  modify_fallback_ws
...
function configure_web() {
  ...
  case $webpage in
  [yY][eE][sS] | [yY])
    wget -O web.tar.gz https://raw.githubusercontent.com/schild/Xray_onekey/main/basic/web.tar.gz
